...
const { parse: parseGql } = require('graphql')


module.exports = function(source, persistFlag = false) {
	const shouldPersist = persistFlag === true

	if (!shouldPersist) this.cacheable()

	const queryMap = {}
	const graphqlDocument = !!source
		? parseGql(source)
		: { definitions: [] }

	// go through the definitions
	for (const def of graphqlDocument.definitions) {
		const operationName = def.name.value
		if (!operationName) throw new Error(`no name: ${def}`)

		// grab the source for this chunk using the loc
		const replaceRegex = /\s+/g
		const operationSource = source.slice(def.loc.start, def.loc.end).replace(replaceRegex, ' ')

		// hash that source
		const hash = base64.encode(crypto.createHash('sha256').update(operationSource, 'utf8').digest())

		if (shouldPersist)
			queryMap[hash] = operationSource
		else
			queryMap[operationName] = hash
	}

	const finalString = JSON.stringify(queryMap)
...
// 				outputCode += `
// 				module.exports["${operationName}"] = oneQuery(doc, "${operationName}")
// 				`
// 			}
// 		}
// 	}
